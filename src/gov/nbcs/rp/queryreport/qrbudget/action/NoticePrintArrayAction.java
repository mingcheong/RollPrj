 ...
 */
public class NoticePrintArrayAction extends CommonAction {
	...
	private class PrintNoticeDialog extends FDialog {
		...
		public PrintNoticeDialog(QrBudget qrBudget) {
			...
			this.dispose();
			this.setModal(true);
			this.setTitle("批量打印一下通知书");
			try {
				init();
			} catch (Exception e) {
				e.printStackTrace();
				JOptionPane.showMessageDialog(qrBudget, "批量打印一下通知书发生错误，错误信息:"
		...
		private void init() throws Exception {
			...
			// 显示树
			DataSet dsDivName = (DataSet) qrBudget.getFtreDivName().getDataSet().clone();
			...
			fbtnPrint.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					NoticePrintAction.printNoticeOperate(ftreDivName);
				}
			});
