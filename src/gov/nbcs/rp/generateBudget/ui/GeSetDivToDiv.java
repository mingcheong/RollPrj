...
public class GeSetDivToDiv extends FDialog {
	...
	public GeSetDivToDiv(List lBudgetData) {
		...
		btnOK.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					GeBudgetAction.getMethod().setDivToDiv(lBudgetData,enToEn_id,enToEn_code,enToEn_name);
					JOptionPane.showMessageDialog(Global.mainFrame,
							"设置归口单位成功！");
					setVisible(false);
				} catch (Exception e1) {
	...
	private void initData() {
		try {
			...
			this.treeDiv = new CustomTree("单位信息表", this.dsDiv, "En_ID",
					"CODE_NAME", "PARENT_ID", null, "DIV_CODE", false);
