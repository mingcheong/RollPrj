...
public class QrBudgetBO implements IQrBudget {
	...
	public DataSet getDivDataPop(String sYear, int batch_no) throws Exception {
		...
		String sSql = " select * from vw_fb_division ENTAB  where is_leaf = 1 and set_year="
				+ sYear
				+ " "
				+ swhere
				+ " union all "
	...
	private String getAuditDivFilter(int batch_no) {
		...
		sql
				.append(" select 1 from rp_xmjl b where (b.div_code = a.div_code or substring(b.div_code,1,3)=a.div_code)");
	...
	public DataSet findProjectByPara(String[] divCodes, Map parMap,
			String[] kmxz, String setYear) throws Exception {
		DataSet dsData = DataSet.create();
		StringBuffer sql = new StringBuffer();
		sql.append("select * from vw_rp_xmcx a where 1=1 and a.set_year='"
				+ setYear + "' ");
		...
		if (!"null".equals(fxmsx)) {
			String xmsx = DBSqlExec
					.getStringValue("select chr_code from dm_xmsx where chr_id='"
							+ fxmsx + " and set_year=" + setYear + " ");
		...
		if ((kmxz != null) && (!kmxz.equals("null"))) {
			sql
					.append(" and a.xmxh in (select xmxh from rp_xmjl_km  where kmdm in(");
	...
	public DataSet find_alllife(String[] divCodes, Map parMap, String[] kmxz,
		...
		sql
				.append("  else SUBSTR(to_char(to_number(pay_monty / (f11_alls + sb_333) / 100)), ");
		sql
				.append("0,INSTR(to_char(to_number(pay_monty / (f11_alls + sb_333) / 100)),'.') + 2) || '%'  end pay_plan ");
		...
		sql
				.append(" b.en_code,b.en_name,sum(b.pay_money) as pay_monty,bis_name from vw_pay_voucher@");
		sql.append(dblink);
		sql.append(" b where b.bi_code like '002%' and b.set_year = ");
		sql.append(setYear);
		sql
				.append("   group by en_id, en_code, en_name, bis_name  order by en_code, bis_name) c");
		sql
				.append("   on a.en_id = c.en_id and a.xmmc = c.bis_name) a where 1=1");
	...
	public void insertTempBudget(String setYear, String batchNo,
			String dateType, String wheresql) throws Exception {
		List list = new ArrayList();
		StringBuffer sqlpost = new StringBuffer();
		sqlpost.append("delete from fb_p_base where set_year='" + setYear
				+ "' and batch_no='" + batchNo + "' and data_type='" + dateType
				+ "'" + wheresql + "");
		StringBuffer sql = new StringBuffer();
		sql
				.append("insert into fb_p_base (select * from vw_fb_p_base where set_year='"
						+ setYear
						+ "' and batch_no='"
						+ batchNo
						+ "' and data_type='" + dateType + "'" + wheresql + ")");
		StringBuffer sqlpostDetail = new StringBuffer();
		sqlpostDetail.append("delete from fb_p_detail_pfs where set_year='"
				+ setYear + "' and batch_no='" + batchNo + "' and data_type='"
				+ dateType + "'" + wheresql + "");
		StringBuffer sqlpDetail = new StringBuffer();
		sqlpDetail
				.append("insert into fb_p_detail_pfs (select * from vw_fb_p_detail_pfs where set_year='"
						+ setYear
						+ "' and batch_no='"
						+ batchNo
						+ "' and data_type='" + dateType + "'" + wheresql + ")");
	...
	public DataSet findRPBUDGET(String[] divCodes, String dataType,
			String fpType, String setYear, String rgCode) throws Exception {
		DataSet dsData = DataSet.create();
		StringBuffer sql = new StringBuffer();
		sql
				.append("SELECT A.SET_YEAR,A.RG_CODE,A.EN_ID,A.DIV_CODE,A.DIV_NAME,A.XMXH AS PRJ_ID,A.XMXH,");
		sql
				.append("A.XMBM AS PRJ_CODE,A.XMMC AS PRJ_NAME,nvl(F.F2 + F.F3 + F.F6 + F.F7,0) as SMONEY,A.NODE_NAME,A.GKDW_CODE FROM");
		...
		sql
				.append(" A.SET_YEAR = F.SET_YEAR AND F.SB_CODE = '333' AND F.IS_IDX IS NULL");
		...
		if (divCodes != null && divCodes.length > 0) {
			...
			sql.append(" AND A.DIV_CODE IN (").append(dCodes.substring(1))
					.append(")");
		}
		...
		if (fpType.equals("1")) {
			...
			if (dataType.equals("1"))
				sql.append(" AND A.NODE_CODE = 10");
			else
				sql.append(" AND A.NODE_CODE != 10");
		}

		sql.append(" and a.c6 = '002'");
		DBSqlExec.getDataSet(sql.toString(), new Object[] { setYear, rgCode },
				dsData);
		return dsData;
	}
