...
public class QrBudgetBO
  implements IQrBudget
{
  ...
  public DataSet getDepToDivData(String sYear, int levl, boolean isRight, int batch_no)
    throws Exception
  {
    String sSql = "";
    if (isRight) {
      ...
      sSql = " select distinct dep_id as EN_ID,dep_id as EID,parent_ID,dep_code as Div_Code,dep_name as Div_Name,dep_name as code_name from vw_fb_department a  where  exists (select 1  from fb_u_dePtodiv ENTAB where ENTAB.dep_code = a.Dep_code and a.set_year=ENTAB.set_year " + swhere + " )" + " and a.set_year =" + sYear + " union all " + " select distinct a.EN_ID,dep_ID||a.EN_ID as EID,case when a.Parent_id is null then dep_ID else a.parent_Id end as parent_Id,a.Div_code,a.div_name,code_name" + " from vw_fb_division a,fb_u_DepToDiv ENTAB " + " where a.set_year = ENTAB.set_year and a.div_code = ENTAB.div_code and a.set_year=" + sYear + " and a.level_num <= " + String.valueOf(levl) + " " + swhere;
    }
    else
    {
      sSql = " select distinct dep_id as EN_ID,dep_id as EID,parent_ID,dep_code as Div_Code,dep_name as Div_Name,dep_name as code_name from vw_fb_department a  where  exists (select 1  from fb_u_dePtodiv ENTAB where ENTAB.dep_code like a.Dep_code||'%' and a.set_year=ENTAB.set_year) and a.set_year =" + sYear + " union all " + " select distinct a.EN_ID,dep_ID||a.EN_ID as EID,case when a.Parent_id is null then dep_ID else a.parent_Id end as parent_Id,a.Div_code,a.div_name,code_name" + " from vw_fb_division a,fb_u_DepToDiv ENTAB " + " where a.set_year = ENTAB.set_year and a.div_code = ENTAB.div_code and a.set_year=" + sYear;
    }
  ...
  public DataSet getDivDataPop(String sYear, int batch_no)
    throws Exception
  {
    IControlDictionary iCDictionary = (IControlDictionary)SessionUtil.getServerBean("sys.controlDictionaryService");
    ...
    String sSql = " select * from vw_fb_division ENTAB  where is_leaf = 1 and set_year=" + sYear + " " + swhere + " union all " + " select * from vw_fb_division tab " + " where tab.is_leaf <> 1 and tab.set_year=" + sYear + " and " + " exists (select 1 from vw_fb_division ENTAB where ENTAB.div_code like tab.div_code||'%'" + " and ENTAB.is_leaf = 1 and ENTAB.set_year=" + sYear + " " + swhere + ")";
  ...
  public DataSet findProjectByPara(String[] divCodes, Map parMap, String[] kmxz, String setYear)
    throws Exception
  {
    DataSet dsData = DataSet.create();
    StringBuffer sql = new StringBuffer();
  ...
  public DataSet find_alllife(String[] divCodes, Map parMap, String[] kmxz, String setYear)
  throws Exception
{
  ...
  public String findkmdm(String xmxh) throws Exception {
    ...
    while (ds.next()) {
      ...
      value.append(ds.fieldByName("fullname").getString()); continue;
  ...
  public void gunDongNextyear(String xmbms, String[] xmxhs, String UserCode, String setYear, String rgCode) throws Exception {
    ...
    for (int i = 0; i < xmxhs.length; i++)
    {
      ...
      StringBuffer sql = new StringBuffer();
      sql.append("insert into rp_xmjl (select '" + year + "', '" + xmxh + "', XMBM, XMMC, C1, C2, C3, C4, C5, C6, C7, C8, C9, C10, EN_ID, RG_CODE, '" + UserCode + "', sysdate, '" + UserCode + "', sysdate, 1, DIV_CODE, DIV_NAME, CS_SORT, EN_SORT, GKDW_ID, GKDW_CODE,ACCTS,GKDW_NAME ");
      ...
      sql3.append("select " + year + ", newid, '" + xmxh + "', YSJC_DM, BS_ID, BSI_ID, F1, F2, F3, F4, F5, F6, F7, F8, EN_ID, RG_CODE, SB_TYPE, WSZT_DM, '" + UserCode + "', sysdate, '" + UserCode + "', sysdate, BZ, SB_CODE, TOTAL_SUM,' ',' ',' ',f9,f10,f11,f12 from ");
      sql3.append("rp_xmsb where sb_code='111' ");
      sql3.append("and xmxh = '" + xmxhs[i] + "')");
      StringBuffer sql4 = new StringBuffer();
      sql4.append("insert into rp_xmsb (");
      sql4.append("select " + year + ", newid, '" + xmxh + "', YSJC_DM, BS_ID, BSI_ID, 0, 0, 0, 0, 0, 0, 0, 0, EN_ID, RG_CODE, SB_TYPE, WSZT_DM, '" + UserCode + "', sysdate, '" + UserCode + "', sysdate, BZ, SB_CODE, 0,' ',' ',' ',0,0,0,0 from ");
      sql4.append("rp_xmsb where sb_code not in ('111','222') ");
      sql4.append("and xmxh = '" + xmxhs[i] + "')");
      StringBuffer sql5 = new StringBuffer();
      sql5.append("insert into rp_xmsb (");
      sql5.append("select " + year + ", newid, '" + xmxh + "', '', ' ', ' ', sum(nvl(f1,0)), sum(nvl(f2,0)), sum(nvl(f3,0)), 0, 0, sum(nvl(f6,0)), sum(nvl(f7,0)), sum(nvl(f8,0)), ' ', '" + rgCode + "', '已安排数', '', '" + UserCode + "', sysdate, '" + UserCode + "', sysdate, '', '222', sum(TOTAL_SUM),'','','',sum(nvl(f9,0)),sum(nvl(f10,0)),sum(nvl(f11,0)),sum(nvl(f12,0)) from ");
  ...
  public void gunDongNextyearLXXM(String xmbms, String[] xmxhs, String UserCode, String setYear, String rgCode) throws Exception {
    ...
    for (int i = 0; i < xmxhs.length; i++)
    {
      xmxh = UUIDRandom.generate();
      StringBuffer sql = new StringBuffer();
      sql.append("insert into rp_xmjl (select '" + year + "', '" + xmxh + "', XMBM, XMMC, '" + year + "', '" + year + "', C3, C4, C5, C6, C7, C8, C9, C10, EN_ID, RG_CODE, '" + UserCode + "', sysdate, '" + UserCode + "', sysdate, 1, DIV_CODE, DIV_NAME, CS_SORT, EN_SORT, GKDW_ID, GKDW_CODE,ACCTS,GKDW_NAME,'' ");
      sql.append(" from rp_xmjl where xmxh = '" + xmxhs[i] + "')");
      ...
      StringBuffer sql3 = new StringBuffer();
      sql3.append("insert into rp_xmsb (");
      sql3.append("select " + year + ", newid, '" + xmxh + "', YSJC_DM, BS_ID, BSI_ID, F1, F2, F3, F4, F5, F6, F7, F8, EN_ID, RG_CODE, SB_TYPE, WSZT_DM, '" + UserCode + "', sysdate, '" + UserCode + "', sysdate, BZ, SB_CODE, TOTAL_SUM,ysjc_mc,acct_name,acct_name_jj,f9,f10,f11,f12 from ");
  ...
  public void finishSameBudget(String xmbms, String xmxhs, String userCode, String setYear, String rgCode)
    throws Exception
  {
    List list = new ArrayList();
  ...
  public void insertTempBudget(String setYear, String batchNo, String dateType, String wheresql)
    throws Exception
  {
    List list = new ArrayList();
    StringBuffer sqlpost = new StringBuffer();
    ...
    throws Exception
  {
    DataSet dsData = DataSet.create();
    StringBuffer sql = new StringBuffer();
    sql.append("select * from VW_RP_P_BUDGET a where 1=1 and a.set_year='" + setYear + "' ");
