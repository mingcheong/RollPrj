...
public class SbjlprojectUI extends RpModulePanel implements PrjActionUI,PrjAuditActionUI
{
	...
	private FPanel getBasePanel() throws Exception
	{
		...
		try
		{
			...
			fxmsx.addValueChangeListener(new ValueChangeListener()
			{
				public void valueChanged(ValueChangeEvent e)
				{
					...
					try
					{
						DataSet ds1 = DBSqlExec.client().getDataSet("select * from dm_xmsx  where set_year=" + Global.loginYear);// chr_id
						// ='"
						// +
						// fxmsxStr[i]
						// +
		...
		xmxx.addControl(getPrjDownPanel(), new TableConstraints(2, 1000, true, true));

		tbPrj = new CustomTable(new String[] { "单位编码", "单位名称", "项目名称", "起始年度", "结束年度", "执行周期", "项目分类", "项目属性", "科目" }, new String[] { "div_code", "div_name", "xmmc", "c1", "c2", "c3", "c4", "c5",
		...
		tbPrj.getTable().addMouseListener(new TableClickListener());
		...
		// RowPreferedLayout layff = new RowPreferedLayout(4);
		...
		pnlfftop.addControl(getQueryType(), new TableConstraints(1, 1, false, true));
			...
			if (tbPrj.getTable() != null)
			{
				...
				try
				{
					if (tbPrj.getDataSet() != null && !tbPrj.getDataSet().isEmpty() && tbPrj.getDataSet().gotoBookmark(bmk))
					{
						...
						{
							setPanelValue(ds.fieldByName("xmbm").getString(), ds.fieldByName("xmmc").getString(), ds.fieldByName("c1").getString(), ds.fieldByName("c2").getString(), ds.fieldByName(
									"c3").getString(), ds.fieldByName("c4").getString(), ds.fieldByName("c5").getString(), ds.fieldByName("c6").getString(), ds.fieldByName("accts").getString(), ds
									.fieldByName("sb_ly").getString());

							tpaa.setAuditXmbm(ds.fieldByName("xmbm").getString());

							tpaa.setAuditXmxh(ds.fieldByName("xmxh").getString());
							tpaa.setEn_code(tbPrj.getDataSet().fieldByName("div_code").getString());
							tpaa.setEn_id(tbPrj.getDataSet().fieldByName("en_id").getString());
							tpaa.setTableData();
						}
						cbYearBegin.setEnabled(false);
						cbYearEnd.setEnabled(false);
	...
	private void refreshPrjData(String set_year)
	{
		try
		{
			StringBuffer cond = new StringBuffer();
			cond.append(" AND a.SET_YEAR = " + set_year);
			...
			DataSet data = PrjInputStub.getMethod().getPrjSbshCreateInfo(cond.toString(), flowstatus, GlobalEx.getModuleId());
	...
	public void doAdd()
	{
		...
		try
		{
			...
			if (nodeSel != null && nodeSel.isLeaf())
			{
				...
				String aPrjCode = PrjInputStub.getMethod().getXmbm(treeEn.getDataSet().fieldByName("div_code").getString(), Global.loginYear);
				...
				tbPrj.getDataSet().fieldByName("sb_ly").setValue(fSbly.getValue() == null ? "" : fSbly.getValue().toString());
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (((Boolean) check).booleanValue())
				{
					if (tbPrj.getDataSet().gotoBookmark(tbPrj.rowToBookmark(i)))
					{
						...
						listSql.add("delete from rp_xmjl_km_divsb where xmxh = '" + xmxh + "' and set_year ='" + Global.loginYear + "'");
						listSql.add("delete from rp_xmjl_divsb where xmxh = '" + xmxh + "' and set_year ='" + Global.loginYear + "'");
	...
	private boolean checkPrjname(String xmmc, String div_code, String xmbm) throws Exception
	{
		String sql = "select count(1) from rp_xmjl_divsb where xmmc='" + xmmc + "' and div_code=" + div_code + " and xmbm !=" + xmbm;
	...
	public void doSave()
	{
		...
		try
		{
			if (checkPrjname(tfPrjName.getValue().toString(), tbPrj.getDataSet().fieldByName("div_code").getString(), tbPrj.getDataSet().fieldByName("xmbm").getString()))
			{
				JOptionPane.showMessageDialog(Global.mainFrame, "保存失败，项目名称重复！");
				return;
			}
		}
		catch (Exception e1)
		...
		try
		{
			if (tbPrj.getDataSet().gotoBookmark(bmk))
			{
				...
				setPanelValue(ds.fieldByName("xmbm").getString(), ds.fieldByName("xmmc").getString(), ds.fieldByName("c1").getString(), ds.fieldByName("c2").getString(), ds.fieldByName("c3")
						.getString(), ds.fieldByName("c4").getString(), ds.fieldByName("c5").getString(), ds.fieldByName("c6").getString(), ds.fieldByName("accts").getString(), ds
						.fieldByName("sb_ly").getString());
			}
	...
	private InfoPackage saveData()
	{
		...
		try
		{
			...
			String xmsxStr = fxmsx.getText() == null ? "" : fxmsx.getText().toString();
	...
	public void setButtonStateUn()
	{
		...
		for (int i = 0; i < controls.size(); i++)
		{
			...
			if ("生成项目".equals(btnGet.getText()))
			{
				btnGet.setEnabled(false);
			}
			if ("退回".equals(btnGet.getText()))
			{
				btnGet.setEnabled(false);
			}
			...
			if ("撤消送审".equals(btnGet.getText()))
			{
				btnGet.setEnabled(false);
			}
			if ("撤消生成".equals(btnGet.getText()))
			{
				btnGet.setEnabled(false);
			}
		}
	...
	public void setButtoncx(boolean isEditState)
	{
		List controls = this.getToolbarPanel().getSubControls();
		for (int i = 0; i < controls.size(); i++)
		{
	...
	public void setButtonStateKZ(boolean isEditState)
	{
		List controls = this.getToolbarPanel().getSubControls();
		for (int i = 0; i < controls.size(); i++)
		{
	...
	public void doSendToAudit()
	{
		...
		try
		{
			...
			String msg = PrjInputStub.getMethod().doshSend(xmxhs, GlobalEx.getCurrRegion(), Global.loginYear, GlobalEx.getModuleId());
			if ("".equals(msg))
				JOptionPane.showMessageDialog(null, "送审成功");
			refreshPrjData();
		}
		catch (Exception e)
		{
	...
	public void doBackToAudit()
	{
		...
		try
		{
			...
			int type = 1;
			PrjInputStub.getMethod().sendbackAudit(xmxhs, GlobalEx.getCurrRegion(), Global.loginYear, GlobalEx.getModuleId(), type);

			JOptionPane.showMessageDialog(null, "撤消送审成功");
			refreshPrjData();
		...
		{
			e.printStackTrace();
			ErrorInfo.showErrorDialog(e, "撤消送审失败");
	...
	public void doUpdatedlg()
	{
		...
		if (rows.length == 0)
		{
			JOptionPane.showMessageDialog(Global.mainFrame, "请选择申报记录");
			return;
		}
		int j = 0;
		try
		{
			j = DBSqlExec.client().getRecordCount(
					"rp_xmjl",
		...
		if (j > 0)
		{
			JOptionPane.showMessageDialog(Global.mainFrame, "此项目名称已经存在，请修改");
			return;
			...
			for (int i = 0; i < rows.length; i++)
			{
				...
				if (tbPrj.getDataSet().gotoBookmark(bmk))
				{
					try
					{
						j = DBSqlExec.client().getRecordCount(
								"rp_xmjl",
								"xmmc='" + Common.nonNullStr(tbPrj.getDataSet().fieldByName("xmxh").getString()) + "' and xmxh!='" + tbPrj.getDataSet().fieldByName("xmxh").getString()
										+ "'and div_code='" + tbPrj.getDataSet().fieldByName("gkdw_code").getString() + "' and set_year =" + Global.loginYear);
						list.add(tbPrj.getDataSet().fieldByName("xmxh").getString());
					}
					catch (Exception ee)
					{
						j = 0;
					...
					if (j > 0)
					{
						JOptionPane.showMessageDialog(Global.mainFrame, "" + tbPrj.getDataSet().fieldByName("xmmc").getString() + "已经存在 生成失败");
						continue;

					}
					prjcode = dto.getNewPrjCodeXmsh(tbPrj.getDataSet().fieldByName("div_code").getString(), i);
					xmxhstr = tbPrj.getDataSet().fieldByName("xmxh").getString();
					PrjInputStub.getMethod().doPrjCreateInfo(Global.loginYear, xmxhstr, prjcode, GlobalEx.getCurrRegion(), null, null, null);

				}
				else
					continue;
			}
			PrjInputStub.getMethod().doshSend(list, GlobalEx.getCurrRegion(), Global.loginYear, GlobalEx.getModuleId());

			JOptionPane.showMessageDialog(Global.mainFrame, "建立项目成功");
			refreshPrjData();
		}
		catch (Exception e)
		{
			e.printStackTrace();
			ErrorInfo.showErrorDialog(e, "建立项目失败");
	...
	public void doExport()
	{
		...
		try
		{
			...
			if (!xmxhs.equals(""))
			{
				...
				{
					xmmcs = xmmcs.substring(0, xmmcs.length() - 1);
					JOptionPane.showMessageDialog(null, "以下项目已经被使用，不能撤消\n" + xmmcs);
					return;
				}
			}
			if (JOptionPane.showConfirmDialog(Global.mainFrame, "确认撤消所选项目？", "提示", JOptionPane.OK_CANCEL_OPTION) != JOptionPane.OK_OPTION) { return; }
			...
			PrjInputStub.getMethod().sendbackAudit(list, GlobalEx.getCurrRegion(), Global.loginYear, moduleid, 1);
			refreshPrjData();
			JOptionPane.showMessageDialog(null, "撤销建立成功");
		}
		catch (Exception e)
		{
			// TODO Auto-generated catch block
			JOptionPane.showMessageDialog(null, "撤消建立失败");
			e.printStackTrace();
		}
	}
	...
	public void backToAuditback()
	{
		...
		try
		{
			...
			PrjInputStub.getMethod().BackAuditInfoByDiv(3, prjcodes, GlobalEx.user_code, GlobalEx.getCurrRegion(), moduleid, Global.loginYear);
			JOptionPane.showMessageDialog(Global.mainFrame, "撤销退回成功");
			refreshPrjData();
		}
		catch (Exception e)
		{
