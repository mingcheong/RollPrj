	...
	public void refreshprj()
	{
		...
		try
		{
			...
			String sqlString = "SELECT A.XMXH,B.XMXH LOCK_XMXH,A.RG_CODE,A.SET_YEAR,A.XMBM,A.XMMC,A.DIV_CODE,A.DIV_NAME,"
					+ "A.MONEY1,A.MONEY2,decode(B.IS_LOCK,'1','是','否')  IS_LOCK FROM (select a.*,c.f2 as MONEY1,c.f3 as MONEY2 from RP_XMJL a inner join rp_xmsb c on a.xmxh = c.xmxh and a.set_year = c.set_year where c.sb_code = '333' " + filter.toString() + ") A" + "　 LEFT JOIN  RP_XMLOCK B ON A.XMXH = B.XMXH ";
