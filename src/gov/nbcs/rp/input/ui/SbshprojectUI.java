...
public class SbshprojectUI extends RpModulePanel implements PrjActionUI
{
	...
	private void refreshPrjData()
	{
		...
		if (treeEn.getSelectedNode() != null)
		{
			...
			try
			{
				...
				DataSet data = PrjInputStub.getMethod().getPrjSbshCreateInfo(filter.toString(), flowstatus, GlobalEx.getModuleId());
							...
							if (tbPrj.getDataSet() != null && !tbPrj.getDataSet().isEmpty() && tbPrj.getDataSet().gotoBookmark(bmk))
							{
								...
								if (!ds.isEmpty() && !ds.bof() && !ds.eof())
								{
									setPanelValue(ds.fieldByName("xmbm").getString(), ds.fieldByName("xmmc").getString(), ds.fieldByName("c1").getString(), ds.fieldByName("c2").getString(), ds
											...
											"accts").getString(), ds.fieldByName("sb_ly").getString());
	...
	public void doUpdatedlg()
	{
		...
		try
		{
			...
			JOptionPane.showMessageDialog(null, "建立项目成功");
			refreshPrjData();
		}
		catch (Exception e)
		{
	...
	public void doAuditToBack()
	{
		// if (!GlobalEx.isFisVis()) {
		// // 只有单位用户才能操作
		// JOptionPane.showMessageDialog(Global.mainFrame, "只有财政用户才退回资格");
		// return;
		// }
		int[] rows = tbPrj.getTable().getSelectedRows();
		if (rows.length == 0)
	...
	public void doExport()
	{
		...
		// }
		// }
		// if (JOptionPane.showConfirmDialog(Global.mainFrame, "确认撤消所选项目？",
		...
		// .getString();
		// List listSql = new ArrayList();
		// listSql.add("delete from rp_xmjl_km where xmxh = '"
		// + xmxh + "' and set_year = "
		// + Global.loginYear+" ");
		// listSql.add("delete from rp_xmjl where xmxh = '" + xmxh
		...
		// }
		//			
		// PrjInputStub.getMethod().sendbackAudit(xmxhs,GlobalEx.getCurrRegion(),Global.loginYear);
		// refreshPrjData();
